...
いずれかがコマンドとして見つからない場合、または Node.js が v20 未満だった場合は、
そこで止めて私に報告してください。ここでインストールはしないでください。

報告には次を含めてください。

- 何が足りないのか（コマンドが無いのか、バージョンが古いのか）
- 「直前に環境準備のプロンプトで Node.js を入れたのなら、ターミナルを開き直すと
  解決することがあります。一度閉じて開き直し、もう一度このプロンプトを試してください」
- それでも同じなら、先に環境準備のプロンプトを実行してほしいこと
